...
public function save(Enseignant $enseignant){
    ...
    try{
        // Insertion utilisateur (inchangé - ✅ correct)
        $stmt = $this->db->prepare("INSERT INTO utilisateur(nom, prenom, email, mot_de_passe, statut, role, telephone) VALUES (:nom, :prenom, :email, :mot_de_passe, :statut, :role, :telephone)");
        $result = $stmt->execute([
        ...
        $enseignant->setIdEnseignant($idUtilisateur);
        
        // ✅ CORRECTION: Utiliser "id_enseignant" au lieu de "id_utilisateur"
        $stmt = $this->db->prepare("INSERT INTO enseignant(id_enseignant, grade, specialite) VALUES (:id_enseignant, :grade, :specialite)");
        $result = $stmt->execute([
            ":id_enseignant" => $enseignant->getIdUtilisateur(),  // ou getIdEnseignant()
        ...
        if(!$result){
            $this->db->rollBack();
            throw new \Exception("Failed to save enseignant.");
        }
...
// ✅ CORRECTION: Jointure avec les bonnes colonnes
